...
///
/// Unlinke [`Interval`] and [`Timeout`], this method will not saturate the duration if it exceedes [`MAX_DURATION`].
...
pub fn sleep(dur: Duration) -> Sleep {
    ...
    match millis / LIMIT {
        0 => Sleep(Inner::Single(Timeout::new_with_millis(noop, millis as i32))),
        div => {
            let rem = core::iter::once(Timeout::new_with_millis(noop, (div % LIMIT) as i32));
            #[cfg(feature = "nightly")]
            let map: TimeoutSet = TimeoutSet;
            #[cfg(not(feature = "nightly"))]
            let map: TimeoutSet = Box::new(|_| Timeout::new_with_millis(noop, i32::MAX));

            let iter = (1..div).map(map).chain(rem);
            let current = Timeout::new_with_millis(noop, i32::MAX);
            return Sleep(Inner::Multiple { iter, current });
        }
    }
}
...
enum Inner {
    Single(Timeout<'static, ()>),
    Multiple {
        iter: std::iter::Chain<
            std::iter::Map<std::ops::Range<u128>, TimeoutSet>,
            std::iter::Once<Timeout<'static, ()>>,
        >,
...
impl Future for Sleep {
    ...
    fn poll(
        ...
        return match &mut self.0 {
            Inner::Single(x) => x.poll_unpin(cx),
            Inner::Multiple { iter, current } => match current.poll_unpin(cx) {
                Poll::Ready(_) => match iter.next() {
                    Some(x) => {
                        *current = x;
                        Poll::Pending
                    }
